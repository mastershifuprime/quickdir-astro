...
let searchData: any[] = [];
try {
  searchData = await import("../../../.json/search.json").then(
    (m) => m.default,
  );
} catch {
  // search.json may not exist yet
}
  ...
  useEffect(() => {
    ...
    const updateSelection = () => {
      const searchResultItems =
        document.querySelectorAll("#searchItem");
    ...
    // Listen for Astro page transitions to close search and reset
    const handleSwap = () => {
      searchModal!.classList.remove("show");
      setSearchString("");
    };
